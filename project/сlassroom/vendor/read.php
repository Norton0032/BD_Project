...
$id = $_GET['id'];
$result = mysqli_query($conn, "SELECT * FROM classroom WHERE id_classroom='$id';");
$row = mysqli_fetch_array($result);
$address = $row['address'];
$id_creator_course = $row['id_creator_course'];
$id_renter = $row['id_renter'];
?>
    <div id="info-container">
        <div id="id">ID: <?php echo $id?></div>
        <div id="address">Адресс: <?php echo $row['address']?></div>
        <div id="id_creator_course">Id создателя курса: <?php echo $row['id_creator_course']?></div>
        <div id="id_renter">Id арендодателя: <?php echo $row['id_renter']?></div>
        <div id="links">
            <a href="..">Назад</a>
